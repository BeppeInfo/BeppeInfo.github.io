...
Introspection requests can be used by a client to get detailed information about a variable/object. They are issued when an appropriate frontend receives a **page payload**, that are e.g. returned from the **IPython** kernel on the **execute _reply** for commands like **"<variable_name>?"** or **"<variable_name>??"** ([**Payloads**](http://jupyter-client.readthedocs.io/en/latest/messaging.html#payloads-deprecated) aren't implemented on the **Scilab** kernel yet).
...
<p align="center">
  <img src="/img/qtconsole_compleness.png">
</p>
<p align="center">
  (The kernel dying (heartbeat thread doesn't respond in time) is a bug to be solved)
</p>
